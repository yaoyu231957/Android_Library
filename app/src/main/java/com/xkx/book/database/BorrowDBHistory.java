...
public class BorrowDBHistory extends SQLiteOpenHelper {

    private static final String DB_NAME = "history.db";
    private static final String TABLE_NAME = "borrow_history";
    private static final int DB_VERSION = 1;
    private static BorrowDBHistory mHistory = null;
    private SQLiteDatabase mRDB = null;
    private SQLiteDatabase mWDB = null;

    private BorrowDBHistory(Context context) {
        super(context, DB_NAME, null, DB_VERSION);
    }

    //利用单例模式获取数据库帮助器的唯一实例
    public static BorrowDBHistory getInstance(Context context) {
        if (mHistory == null) {
            mHistory = new BorrowDBHistory(context);
        }
        return mHistory;
    }
    ...
    public SQLiteDatabase openReadLink() {
        if (mRDB == null || !mRDB.isOpen()) {
            mRDB = mHistory.getReadableDatabase();
        }
        return mRDB;
    }
    ...
    public SQLiteDatabase openWriteLink() {
        if (mWDB == null || !mWDB.isOpen()) {
            mWDB = mHistory.getWritableDatabase();
        }
        return mWDB;
    }
